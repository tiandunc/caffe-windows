...
namespace caffe {
	...
	void NormalizeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		...
		squared_.Reshape(bottom[0]->num(), bottom[0]->channels(),
			bottom[0]->height(), bottom[0]->width());
		//sum_multiplier_.Reshape(dim_, 1， 1， 1);
		sum_multiplier_.Reshape(dim_, 1, 1, 1);
		norm_.Reshape(bottom[0]->num(), 1, 1, 1);
		diff_data_inner_product_.Reshape(bottom[0]->num(), 1, 1, 1);
		Dtype* sum_multiplier_data = sum_multiplier_.mutable_cpu_data();
		for (int i = 0; i < dim_; i++)
			sum_multiplier_data[i] = Dtype(1.0);
	}
